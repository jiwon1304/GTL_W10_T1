...
namespace ImGui {
    ...
    IMGUI_API void  RenderNeoSequencerTopBarOverlay(float zoom, float valuesWidth,float_t startFrame, float_t endFrame, float_t offsetFrame, const ImVec2 &cursor, const ImVec2& size, ImDrawList * drawList = nullptr, bool drawFrameLines = true, bool drawFrameText = true, float maxPixelsPerTick = -1.0f);
    ...
    IMGUI_API void  RenderNeoTimeline(bool selected,const ImVec2 & cursor, const ImVec2& size, const ImVec4& highlightColor, ImDrawList *drawList = nullptr);
    ...
    IMGUI_API float GetPerFrameWidth(float totalSizeX, float valuesWidth, float_t endFrame, float_t startFrame, float zoom);
}
