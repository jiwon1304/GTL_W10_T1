    ...
    //    if(Attribute->GetAttributeType() == FbxNodeAttribute::eMesh)
        {
            ...
            if (Node->GetMesh())
            {
                ...
                ParseMesh(Mesh, OutRenderData);
                ParseSkinningData(Mesh, OutRenderData);
                // Parse the material
                const int MaterialCount = Node->GetMaterialCount();
                bool bNoMaterial = (MaterialCount == 0);

                // 1. 머티리얼 없는 경우: 기본 머티리얼 추가
                if (bNoMaterial)
                {
                    FMaterialInfo DefaultMaterial;
                    DefaultMaterial.MaterialName = TEXT("DefaultMaterial");
                    ...
                    DefaultSubset.MaterialName = TEXT("DefaultMaterial");
                ...
                else
                {
                    ...
                    {
                        TMap<int32, FMaterialSubset> MaterialSubsetMap;

                        for (int PolygonIndex = 0; PolygonIndex < PolygonCount; ++PolygonIndex)
                        {
                            int MaterialIndex = MaterialLayer->GetIndexArray().GetAt(PolygonIndex);

                            if (!MaterialSubsetMap.Contains(MaterialIndex))
                            {
                                FMaterialSubset NewSubset;
                                NewSubset.MaterialIndex = MaterialIndex;
                                NewSubset.IndexStart = OutRenderData.Indices.Num(); // Start at current index buffer position
                                NewSubset.IndexCount = 0;
                                NewSubset.MaterialName = OutRenderData.Materials.IsValidIndex(MaterialIndex)
                                    ? OutRenderData.Materials[MaterialIndex].MaterialName
                                    : FString::Printf(TEXT("Material_%d"), MaterialIndex);
                                MaterialSubsetMap.Add(MaterialIndex, NewSubset);
                            }

                            for (int VertexIndex = 0; VertexIndex < Mesh->GetPolygonSize(PolygonIndex); ++VertexIndex)
                            {
                                int ControlPointIndex = Mesh->GetPolygonVertex(PolygonIndex, VertexIndex);
                                OutRenderData.Indices.Add(ControlPointIndex);
                                MaterialSubsetMap[MaterialIndex].IndexCount++;
                            }
                        }

                        for (auto& SubsetEntry : MaterialSubsetMap)
                        {
                            OutRenderData.MaterialSubsets.Add(SubsetEntry.Value);
                        }
                    }
                }
            }
...
void FFbxLoader::ParseSkeleton(FbxNode* Node, FSkeletalMeshRenderData& OutRenderData)
{
}
...
void FFbxLoader::ParseMesh(FbxMesh* Mesh, FSkeletalMeshRenderData& OutRenderData)
{
    ...
    // Parse Skinning Data (Bone Indices, Weights, Bind Pose Matrices)
    FbxSkin* Skin = static_cast<FbxSkin*>(Mesh->GetDeformer(0, FbxDeformer::eSkin));
    if (Skin)
    {
        OutRenderData.InverseBindPoseMatrices.SetNum(Skin->GetClusterCount());
        ...
        for (int ClusterIndex = 0; ClusterIndex < Skin->GetClusterCount(); ++ClusterIndex)
        {
            ...
            OutRenderData.InverseBindPoseMatrices[ClusterIndex] = FMatrix::Inverse(ConvertedMatrix);
            ...
            for (int i = 0; i < Cluster->GetControlPointIndicesCount(); ++i)
            {
                ...
                for (int j = 0; j < 4; ++j)
                {
                    ...
                    {
                        OutRenderData.Vertices[ControlPointIndex].BoneIndices[j] = OutRenderData.Bones.Num() - 1;
    ...
    else
    {
        ...
        OutRenderData.InverseBindPoseMatrices.SetNum(1);
        OutRenderData.InverseBindPoseMatrices[0] = FMatrix::Identity;

        // 3. 모든 버텍스에 Root Bone 100% 영향 할당
        for (int32 i = 0; i < OutRenderData.Vertices.Num(); ++i)
        {
            OutRenderData.Vertices[i].BoneIndices[0] = 0;
